...
def filtered(edges, elem, new_elem):
    result = []
    for edg in edges:
        result.append(edg if edg != elem else new_elem)
    return result

def pretty_result(result):
    global vertex_number
    for (elem, _, _) in result:
        vertex_number.add(elem)
    for (elem, _, _) in result:
        if elem > 0:
            if elem in vertex_number:
                new_elem = max(vertex_number) + 1
                new_result = []
                for (vert, sym, edges) in result:
                    new_result.append((vert if vert != elem else new_elem, sym, filtered(edges, elem, new_elem)))
                result = new_result
                vertex_number.add(new_elem)
            else:
                vertex_number.add(elem)
    dict = {}
